//
//  MyColor.m
//
//  Created by  qin on 2017/5/1.
//  Copyright © 2017年  qin. All rights reserved.
//

#import "MyColor.h"

@implementation MyColor
+(UIColor *)redColor
{
    return [UIColor colorWithRed:255.0/255 green:59.0/255 blue:48.0/255 alpha:1.0];
}
+(UIColor *)orangeColor
{
    return [UIColor colorWithRed:255.0/255 green:149.0/255 blue:0/255 alpha:1.0];
}
+(UIColor *)yellowColor
{
    return [UIColor colorWithRed:255.0/255 green:204.0/255 blue:0/255 alpha:1.0];
}
+(UIColor *)greenColor
{
    return [UIColor colorWithRed:76.0/255 green:217.0/255 blue:100.0/255 alpha:1.0];
}
+(UIColor *)tealBlueColor
{
    return [UIColor colorWithRed:90.0/255 green:200.0/255 blue:250.0/255 alpha:1.0];
}
+(UIColor *)blueColor
{
    return [UIColor colorWithRed:0/255 green:122.0/255 blue:255.0/255 alpha:1.0];
}
+(UIColor *)purpleColor
{
    return [UIColor colorWithRed:88.0/255 green:86.0/255 blue:214.0/255 alpha:1.0];
}
+(UIColor *)pinkColor
{
    return [UIColor colorWithRed:255.0/255 green:45.0/255 blue:85.0/255 alpha:1.0];
}
@end
